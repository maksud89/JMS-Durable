...
public class AsynchronousMessageConsumer {
	...
	private static final String DEFAULT_DESTINATION = "jms/topic/myTestTopic";
	...
	//	MessageConsumer consumer = null;
		...
		String destinationString = System.getProperty("destination",
				DEFAULT_DESTINATION);
		destination = (Topic) context.lookup(destinationString);
        ...
        topicSubscriber = session.createDurableSubscriber(destination, "subscriber2");		
		
		connection.start();
		topicSubscriber.setMessageListener(new JmsMessageListener());

		System.out.println("Start listening... press any key to exit");
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		br.read();

		session.unsubscribe("mak");
		session.close();
		connection.close();
